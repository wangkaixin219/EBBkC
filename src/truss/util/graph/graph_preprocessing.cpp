...
void getEidAndEdgeList(graph_t *g, Edge *idToEdge) {
    ...
    g->edge_truss = (vid_t *) malloc(g->m  / 2 * sizeof(vid_t));
    g->edge_rank = (vid_t *) malloc(g->m  / 2 * sizeof(vid_t));
...
#pragma omp parallel for
    for (auto i = 0; i < max_omp_threads; i++) {
        ...
        memset(g->edge_rank + iter_beg, INT_MAX, (iter_end - iter_beg) * sizeof(int));
    }

    g->v_set = vector<vector<vid_t>>(g->m / 2);
    g->__rank = 0;
